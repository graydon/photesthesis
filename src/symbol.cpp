...
namespace {
struct DerefLess {
  bool operator()(std::shared_ptr<std::string> const &x,
...
std::shared_ptr<std::string> Symbol::intern(std::string const &s) {
  ...
  for (auto const &c : s) {
    if (!(isalnum(c) || c == '_')) {
      throw std::runtime_error("Symbol must be alphanumeric-or-underscores");
    }
  }
