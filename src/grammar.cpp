...
Value const &Lit::getValue() const { return mValue; }
...
RuleName const &Ref::getRuleName() const { return mRuleName; };

uint64_t Ref::getTag() const { return mTag; }

std::set<ParamName> Ref::getCtxExt() const { return mCtxExt; }
...
void Context::push(ParamName sym) { mLocalParamNames.emplace_back(sym); }
void Context::push(std::set<ParamName> const &ps) {
  for (auto const &p : ps) {
    push(ParamName(p));
  }
}
void Context::pop(size_t n) {
  while (n != 0) {
    mLocalParamNames.pop_back();
    n -= 1;
  }
}
bool Context::has(ParamName const &s) const {
...
bool Context::has(std::set<ParamName> const &ss) const {
  for (auto const &s : ss) {
    if (!has(s)) {
      return false;
    }
...
LitPtr Grammar::Sym(Symbol const &s) {
  return std::make_shared<const Lit>(Value(s));
}
LitPtr Grammar::Bool(bool s) {
  return std::make_shared<const Lit>(Value::Bool(s));
}
LitPtr Grammar::Int64(int64_t i) {
  return std::make_shared<const Lit>(Value::Int64(i));
}
LitPtr Grammar::Blob(std::vector<uint8_t> const &s) {
  return std::make_shared<const Lit>(Value(std::vector<uint8_t>(s)));
}
LitPtr Grammar::Str(std::string const &s) {
  return std::make_shared<const Lit>(Value(std::string(s)));
}

RefPtr Grammar::Ref(RuleName const &s,
                    std::initializer_list<ParamName> ctxExt) {
  return std::make_shared<const class Ref>(s, ctxExt);
}

void Grammar::addRule(RuleName const &name,
                      std::initializer_list<Production> prods) {
...
std::set<KPath> Grammar::expandKPathPrefix(size_t k, KPath const &prefix,
                                           Context &context,
  ...
  for (auto const &prod : prods) {
    for (auto const &ext : prod.get().mAtoms) {
      ...
      // We only accept non-ref (i.e. literal) extensions at the last step of
      // a k-path. At earlier points in a k-path we require refs.
      if (ref || prefix.size() == k - 1) {
        KPath extended = prefix;
        extended.emplace_back(ext);
        auto expanded = expandKPathPrefix(k, extended, context, pathRoots);
        res.insert(expanded.begin(), expanded.end());
      }
      // If we're at a ref we've not yet started-from, we also start exploring a
      // _new_ k-path starting from this ref.
      if (ref && pathRoots.find(ref) == pathRoots.end()) {
        KPath restarted{ext};
        pathRoots.insert(ref);
        auto expanded = expandKPathPrefix(k, restarted, context, pathRoots);
        res.insert(expanded.begin(), expanded.end());
      }
      if (ref) {
...
std::vector<Production> const &Grammar::getProductions(RuleName rule) const {
...
Grammar::getActiveProductions(RuleName rule, size_t depthLimit,
  ...
  if (activeProds.empty()) {
    if (skippedDueToRefs) {
      ...
      throw std::runtime_error(std::string("no active productions found for ") +
                               rule.getString());
    }
  }
  return activeProds;
}
RefPtr Grammar::getRootRef(RuleName rulename) const {
...
std::set<KPath> Grammar::generateKPathSet(size_t k, RuleName root,
...
extendByCycling(std::set<std::vector<Value>> const &vecs,
  ...
  // std::cout << "cycling extension grew set to " << res.size() << " prefixes"
  //           << std::endl;
  return res;
}

static std::set<Params> extendByCycling(std::set<Params> const &params,
                                        ParamName param,
                                        std::set<Value> const &ext) {
  assert(!params.empty());
  assert(!ext.empty());
  ...
  for (auto prodref : prods) {
    ...
    for (auto const &atom : prod.mAtoms) {
      ...
      if (paths.find(kpath) != paths.end()) {
        // this production covers a k-path -- we need to keep at least one
        // expansion of it.
        paths.erase(kpath);
        kpath.pop_back();
    ...
    for (auto const &atom : prod.mAtoms) {
      ...
      if (auto lit = std::dynamic_pointer_cast<const Lit>(atom)) {
      ...
      } else if (auto ref = std::dynamic_pointer_cast<const class Ref>(atom)) {
        context.push(ref->getCtxExt());
        ...
        std::tie(subKPathCovering, subNonKPathCovering) =
            kPathCoveringOrMinimalExpansion(subPath, depthLimit - 1, context, k,
                                            paths);
        context.pop(ref->getCtxExt().size());
...
std::set<Value> Grammar::kPathCovering(RuleName rule, size_t k,
                                       ParamSpecs const &specs) const {
  Context ctx(specs);
  ...
  size_t depthLimit = k;
  while (!paths.empty()) {
    auto pair = kPathCoveringOrMinimalExpansion({getRootRef(rule)}, depthLimit,
                                                ctx, k, paths);
    if (pair.first.empty()) {
      /*
...
std::set<Params> Grammar::kPathCoverings(size_t k,
                                         ParamSpecs const &specs) const {
  std::set<Params> res;
  for (auto const &spec : specs) {
    ...
    if (res.empty()) {
      ...
      // FIXME: we might want to do a cartesian product here instead of cycling?
      // Or a different N-tuples coverage? It could get expensive. Tradeoffs...
...
// active produtions.
Value Grammar::randomValueFromRule(RuleName rule,
  ...
  if (!prods.empty()) {
    ...
    for (auto atom : prod.mAtoms) {
      if (auto lit = std::dynamic_pointer_cast<const Lit>(atom)) {
        ...
        context.push(ref->getCtxExt());
        Value val = randomValueFromRule(ref->getRuleName(), gen, depth_lim - 1,
                                        context);
        assert(val.isPair());
        Value rest;
...
Plan Grammar::randomlyPopulatePlan(TestName tname, ParamSpecs const &params,
                                   std::default_random_engine &gen,
...
std::set<Plan> Grammar::populatePlansFromKPathCoverings(TestName tname,
