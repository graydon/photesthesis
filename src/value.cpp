...
bool ValueImpl::match() const { return true; }

bool ValueImpl::match(
    std::pair<Value, std::shared_ptr<const PairValue>> &out) const {
  return false;
}
bool ValueImpl::match(Symbol &out) const { return false; }
bool ValueImpl::match(bool &out) const { return false; }
bool ValueImpl::match(int64_t &out) const { return false; }
bool ValueImpl::match(std::vector<uint8_t> &out) const { return false; }
bool ValueImpl::match(std::string &out) const { return false; }

Type Value::getType() const { return mImpl ? mImpl->getType() : Type::Nil; }
...
Value Value::Bool(bool val) {
  return Value(std::make_shared<const BoolValue>(val));
}

Value Value::Int64(int64_t val) {
...
bool Value::match() const { return true; }
bool Value::match(Value &out) const {
  out = *this;
  return true;
};

bool Value::isNil() const { return getType() == Type::Nil; }
bool Value::isPair() const { return getType() == Type::Pair; }
bool Value::isSym() const { return getType() == Type::Sym; }
bool Value::isBool() const { return getType() == Type::Bool; }
bool Value::isInt64() const { return getType() == Type::Int64; }
bool Value::isBlob() const { return getType() == Type::Blob; }
bool Value::isString() const { return getType() == Type::String; }
...
std::istream &operator>>(std::istream &is, Value &val) {
  ...
  case '#': {
    ...
    if (tmp == "#t") {
      ...
      throw std::runtime_error(std::string("unknown special symbol: ") + tmp);
    }
  }
  default: {
...
Type PairValue::getType() const { return Type::Pair; }
...
Type SymValue::getType() const { return Type::Sym; }
SymValue::SymValue(Symbol val) : TypedValue(val) {}

Type BoolValue::getType() const { return Type::Bool; }
BoolValue::BoolValue(bool val) : TypedValue(val) {}

Type Int64Value::getType() const { return Type::Int64; }
Int64Value::Int64Value(int64_t val) : TypedValue(val) {}

Type BlobValue::getType() const { return Type::Blob; }
BlobValue::BlobValue(std::vector<uint8_t> const &val) : TypedValue(val) {}

Type StringValue::getType() const { return Type::String; }
