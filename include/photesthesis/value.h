...
public:
  ...
  // Convenience constructor that builds a Pair-based structure.
  ...
  template <typename T> bool match(T &out) const {
...
public:
  ...
  bool match(T &out) const override {
    out = mValue;
    return true;
  }
...
public:
  ...
  // recursive template call in any chain would be ambiguous if we bottomed out
  // in the empty argpack.
...
template <typename T, typename... Args>
inline bool Value::match(T &head, Args &... tail) const {
...
template <typename T, typename... Args>
inline bool Value::match(T &head, Args const &... tail) const {
  if (auto a = std::dynamic_pointer_cast<const PairValue>(mImpl)) {
    return a->match(head, tail...);
  }
  return false;
...
template <typename T, typename... Args>
inline bool Value::match(T const &head, Args &... tail) const {
  if (auto a = std::dynamic_pointer_cast<const PairValue>(mImpl)) {
    return a->match(head, tail...);
  }
  return false;
...
template <typename T, typename... Args>
inline bool Value::match(T const &head, Args const &... tail) const {
  if (auto a = std::dynamic_pointer_cast<const PairValue>(mImpl)) {
    return a->match(head, tail...);
  }
  return false;
...
template <typename T> inline bool PairValue::match(T &v) const {
  return getValue().first.match(v);
}
template <typename T> inline bool PairValue::match(T const &v) const {
  return getValue().first.match(v);
}

template <typename T, typename... Args>
inline bool PairValue::match(T &head, Args &... tail) const {
  auto &pair = getValue();
...
template <typename T, typename... Args>
inline bool PairValue::match(T const &head, Args &... tail) const {
  auto &pair = getValue();
  return pair.first.match(head) &&
         (!pair.second || pair.second->match(tail...));
}
...
template <typename T, typename... Args>
inline bool PairValue::match(T &head, Args const &... tail) const {
  auto &pair = getValue();
  return pair.first.match(head) &&
         (!pair.second || pair.second->match(tail...));
}
...
template <typename T, typename... Args>
inline bool PairValue::match(T const &head, Args const &... tail) const {
  auto &pair = getValue();
  return pair.first.match(head) &&
         (!pair.second || pair.second->match(tail...));
}
