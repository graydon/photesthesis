...
class Test
{
    ...
    Failures randomlyExpandCorpus(Trajectories&, uint64_t steps,
                                  uint64_t depth);
  ...
  protected:
    ...
    // test's trajectory (which also includes any path-coverage information we
    // can observe, if compiled with instrumentation). Test runs are grouped by
    ...
    // except the word 'track' is put in the transcript instead of 'check',
    // which emphasizes the trajectory-sensitivity to anyone reading the
    // transcript.
    //
  ...
  public:
    ...
    //
    // If the environment variable `PHOTESTHESIS_TEST_HASH` is set to some
    // number, only the transcript labeled with that number will be checked.
    //
    // The corpus will be re-written if any checks fail or the corpus is
    // expanded.
    //
    // `administer` returns a vector of PlanHashes that identify any transcripts
    // that failed.
    //
    // If you want to get a useful and informative test failure signal at an
    // outer level test harness, assert that the return value of this is equal
    // to the empty vector.
